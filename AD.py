...
app = Flask(__name__)
CORS(app, resources={r"/video_feed_2/*": {"origins": "http://127.0.0.1:8000"}})
CORS(app, resources={r"/video_feed_1/*": {"origins": "http://127.0.0.1:8000"}})
CORS(app, resources={r"/video_feed_3/*": {"origins": "http://127.0.0.1:8000"}})
CORS(app, resources={r"/video_feed_4/*": {"origins": "http://127.0.0.1:8000"}})
CORS(app, resources={r"/video_feed_5/*": {"origins": "http://127.0.0.1:8000"}})
CORS(app, resources={r"/video_feed_6/*": {"origins": "http://127.0.0.1:8000"}})

@app.route('/video_feed_1/<camera_id>/<path:video_name>')
def video_feed_1(camera_id,video_name):
    video_path = unquote(video_name)  # Декодирование пути
    return Response(process_network_1(camera_id,video_path), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/video_feed_2/<camera_id>/<path:video_name>')
def video_feed_2(camera_id, video_name):
    video_path = unquote(video_name)  # Декодирование пути
    return Response(process_network_2(camera_id, video_path), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/video_feed_3/<camera_id>/<path:video_name>')
def video_feed_3(camera_id,video_name):
    video_path = unquote(video_name)  # Декодирование пути
    return Response(process_network_3(camera_id,video_path), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/video_feed_4/<camera_id>/<path:video_name>')
def video_feed_4(camera_id,video_name):
    video_path = unquote(video_name)  # Декодирование пути
    return Response(process_network_4(camera_id,video_path), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/video_feed_5/<camera_id>/<path:video_name>')
def video_feed_5(camera_id,video_name):
    video_path = unquote(video_name)  # Декодирование пути
    return Response(process_network_5(camera_id,video_path), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/video_feed_6/<camera_id>/<path:video_name>')
def video_feed_6(camera_id,video_name):
    video_path = unquote(video_name)  # Декодирование пути
    return Response(process_network_6(camera_id,video_path,db_config), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def process_network_1(camera_id,myVideoUse):
    ...
    while not video_finished:    
        ...
        if not ret:
            ...
            continue
        # Изменение размера кадра до 1280x720
        ...
        if not has_accident:
            accidCount = 0
        cv2.putText(frame, f"{camera_id} Cam", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
...
def process_network_2(camera_id, myVideoUse): 
    ...
    classNames = ["person", "bicycle", "car", "motorbike", "aeroplane", "bus", "train", "truck", "boat",
       ...
       #Создание маски
    def create_image(width, height, vertices):
        img = Image.new("RGB", (width, height), "black")
        draw = ImageDraw.Draw(img)
        draw.polygon(vertices, fill="white", outline="white")
        return img
    ...
    vertices = [(500, 200), (100, 450), (700, 450), (700, 200)]

    # Создаем изображение
    img = create_image(width, height, vertices)
    ...
    while not video_finished:
        ...
        cv2.putText(img, f"{camera_id} Limits1: {len(object_ids1)}", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
...
def process_network_3(camera_id,myVideoUse):
    ...
    while not video_finished:
        ...
        framea = cv2.resize(framea, (1020, 500))
        results = model.predict(framea)
        aa = results[0].boxes.data
        a = aa.cpu().detach().numpy()
        px = pd.DataFrame(a).astype("float")
        ...
        for index, row in px.iterrows():
            ...
            if "normal" in c: 
                ...
                cvzone.putTextRect(framea, f'{c}', (x1, y1), 1, 1)
                
            if "fight" in c: 
                cv2.rectangle(framea, (x1, y1), (x2, y2), (0, 0, 250), 2)
                cvzone.putTextRect(framea, f'{c}', (x1, y1), 1, 1)
        cv2.putText(framea, f"{camera_id} Cam", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)    
        _, buffer = cv2.imencode('.jpg', framea)
        frame_bytes = buffer.tobytes()
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + frame_bytes + b'\r\n')
...
def process_network_4(camera_id,myVideoUse):
...
def process_network_5(camera_id,myVideoUse):
    class FireDetection:
...
def process_network_6(camera_id, myVideoUse, db_config): 
    yoloModel = "yolov8l.pt"
    maskPng = f"mask_{camera_id}.png"  # Добавляем camera_id к имени файла маски
    ...
    create_table_crossings_query = """
    CREATE TABLE IF NOT EXISTS object_crossings (
    ...
    """
    try:
        cursor.execute(create_table_crossings_query)
        cursor.execute(create_table_frames_query)
    ...
    # Интересующие нас классы объектов
    interesting_classes = ["car", "truck", "bus", "motorbike"]

    # Создание маски
    def create_image(width, height, vertices):
        img = Image.new("RGB", (width, height), "black")
        draw = ImageDraw.Draw(img)
        polygon_vertices = [(vertices[i], vertices[i + 1]) for i in range(0, len(vertices), 2)]
        draw.polygon(polygon_vertices, fill="white", outline="white")
        return img

    # Получаем размеры видео
    width, height = 1280, 720  # Задаем фиксированные размеры
    ...
    # Основной цикл обработки видеопотока
    video_finished = False
    frame_saved = False  # Флаг для сохранения только одного кадра
    ...
    while not video_finished:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                # Проверка наличия объекта в списке интересующих нас классов и достаточного уровня уверенности
                if currentClass in interesting_classes and conf > 0.3:
                    # Добавление детекции в массив
                    currentArray = np.array([x1, y1, x2, y2, conf])
                    detections = np.vstack((detections, currentArray))
        ...
        for result in resultsTracker:
            ...
            if limits1[0] < cx < limits1[2] and limits1[1] - 15 < cy < limits1[1] + 15:
                if id not in object_ids1:
                    ...
                    # Запись информации о пересечении в базу данных только для интересующих нас классов
                    if currentClass in interesting_classes:
                        insert_query = """
                        INSERT INTO object_crossings (camera_id, object_type, crossing_time)
                        VALUES (%s, %s, %s)
                        """
                        crossing_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                        cursor.execute(insert_query, (camera_id, currentClass, crossing_time))
                        db_connection.commit()

            # Подсчёт времени, проведенного в области limits2 для существующего объекта
            if id in object_times2:
                if time.time() - object_times2[id] > 10:
                    # Окантовка кадра красным цветом толщиной 15 пикселей для отображения
                    img = cv2.rectangle(img, (0, 0), (width - 1, height - 1), (0, 0, 255), 15)
                    if not frame_saved:
                        print(f"Warning: Object ID {id} spent more than 10 seconds in limits2!")
                        # Сохранение кадра с красной окантовкой
                        img_with_border = img.copy()
                        image_path = f"zators/{camera_id}_{int(time.time())}.jpg"
                        cv2.imwrite(image_path, img_with_border)
                        # Проверка, что файл сохранился
                        if os.path.exists(image_path):
                            print(f"Saved frame to {image_path}")
                            # Запись информации в базу данных
                            insert_query_frame = """
                            INSERT INTO object_frames (camera_id, frame_time, image_path)
                            VALUES (%s, %s, %s)
                            """
                            frame_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                            cursor.execute(insert_query_frame, (camera_id, frame_time, image_path))
                            db_connection.commit()
                            frame_saved = True
                        else:
