...
# Инициализируем словарь для хранения идентификаторов объектов
objects = {}
...
while True:    
    ...
    for index, row in px.iterrows():
        ...
        if "Accident" in c:
            ...
            cv2.putText(frame, f'ID: {dtp_count}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            ...
            # Присваиваем уникальный идентификатор объекту
            object_id = f"Accident_{dtp_count}"
            objects[object_id] = {'type': 'Accident', 'bbox': (x1, y1, x2, y2)}
        ...
        if "Car" in c: 
            ...
            cv2.putText(frame, f'ID: {index}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cvzone.putTextRect(frame, f'{c}', (x1, y1), 1, 1)
            
            # Присваиваем уникальный идентификатор объекту
            object_id = f"Car_{index}"
            objects[object_id] = {'type': 'Car', 'bbox': (x1, y1, x2, y2)}
        ...
        if "Sign" in c: 
            ...
            # Присваиваем уникальный идентификатор объекту
            object_id = f"Sign_{index}"
            objects[object_id] = {'type': 'Sign', 'bbox': (x1, y1, x2, y2)}
