...
while True:    
    ...
    for index, row in px.iterrows():
        ...
        if "Accident" in c:
            ...
            if index in object_ids:
                cv2.putText(frame, f'ID: {object_ids[index]}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            ...
            if index not in object_ids:
                object_ids[index] = len(object_ids)
        ...
        if "Car" in c: 
            ...
            if index in object_ids:
                cv2.putText(frame, f'ID: {object_ids[index]}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cvzone.putTextRect(frame, f'{c}', (x1, y1), 1, 1)
